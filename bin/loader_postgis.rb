...
def main conn, layers, flags = {}
  ...
  layers.each do |k, v|
    ...
    layername = v[:layername].downcase.strip
...
begin
  flags = {
    ...
    :verbose => true,
    :register => false,
    :register_table => 'layers',
    :datadir => '/var/geomdtk/current/workspace',
    :host => GeoMDTK::Config.postgis.host || 'localhost',
    :port => GeoMDTK::Config.postgis.port || '5432',
    :username => GeoMDTK::Config.postgis.username || 'postgres',
    :password => GeoMDTK::Config.postgis.password || nil,
    :database => GeoMDTK::Config.postgis.database || 'postgres',
...
"
    ...
    %w{host port username password database schema}.each do |k|
  ...
  dbflags = flags.reject {|k,v| ([k.to_s] & %w{host port username password database}).empty?}
  ap({:flags => flags, :dbflags => dbflags, :dbflags_class => dbflags.class})
  ...
  conn = ActiveRecord::Base.establish_connection(
          :adapter  => 'postgresql',
          :host     => dbflags[:host],
          :port     => dbflags[:port].to_i,
          :database => dbflags[:database],
          :username => dbflags[:username],
          :password => dbflags[:password]
      )
  
  # conn = PG.connect dbflags
  # conn.trace($stderr) if flags[:debug]
  ap({:conn => conn}) if flags[:debug]
  begin
    # if flags[:debug]
    ...
    if flags[:register]
      # result = conn.query('SELECT COUNT(*) FROM pg_tables WHERE schemaname = $1 and tablename = $2', [flags[:schema], flags[:register_table]])
      # ap({:result => result.values, :first => result.getvalue(0,0), :klass => result.getvalue(0,0).class})
      ...
      #     (
      #       druid character(11) NOT NULL PRIMARY KEY,
